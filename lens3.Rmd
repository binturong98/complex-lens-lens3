...
edge_supplement_leg <- plot_grid(edge_supplement_plot, x.grob,
                                 ncol = 1, rel_heights = c(1, .1))
plot_grid(edge_supplement_leg)

# Print graph
cairo_pdf(file = "Figures/edge_curve_supplement.pdf", width = 12,  height = 8) # cairo_pdf is able to handle special characters
grid.arrange(arrangeGrob(edge_supplement_leg, 
                         left = y.grob,
                         bottom = legend))
dev.off()
```
...
# Using an ANOVA test
res_qPCR_aov <- compare_means(log_fold_gene_expression ~ probe, data = qpcr_data, method = 'anova')
res_qPCR_aov
