...
# Trying custom facet grid labels using the labeller function
probe.labs <- c("control" = "Control","lens3_191" = "Probe 1", "lens3_249" = "Probe 2")
...
latency_horizontal_pooled <- behaviour_data %>%
  ...
  scale_x_discrete(labels = c(expression("control" = Control, "lens3" = italic(lens3)~RNAi))) +
  scale_y_continuous(limits = c(0, 26), breaks = c(0, 5, 10, 15, 20, 25)) +
  labs(title=NULL,x= "", y = "Latency (min)") +
  geom_jitter(shape = 19, size = 3.5, alpha = 0.7, position = position_jitter(0.1)) + 
...
#print graph
pdf(file = "Figures/latency.pdf", width = 15,  height = 9)
grid.arrange(arrangeGrob(latency_plot, left = y.grob)) #adding common y axis label to plot
dev.off()
