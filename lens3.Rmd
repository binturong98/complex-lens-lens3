...
```{r LE2 edge sharpness regression figure}
...
```{r supplementary edge sharpness curves}

fig.a <- LE1_edge_plot +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank()) + 
  theme(legend.position = "none",
        plot.margin = unit(c(0, 0, 1, 0.5), "cm")) #The order of edges for plot.margin is unit(c(top, right, bottom, left), units)
...
Comparing maximum edge sharpness values between controls and tests. This usually corresponds to the second peak/image. However, sometimes the first peak has higher edge sharpness in the knockdowns; That's some of the eyes of some individuals (3/10) for Probe 1 and for Probe 2 (4/10). In some eyes of Probe 2 individuals (2/10), best focused images could not be detected in the edge sharpness plots or manually
...
```{r supplementary maximum edge sharpness figure}

fig.a <- LE1_max_edge +
  theme(axis.title.y = element_blank()) + 
  theme(legend.position = "none",
        plot.margin = unit(c(0, 0, 0.5, 0.5), "cm")) #The order of edges for plot.margin is unit(c(top, right, bottom, left), units)
...
```{r LE2 back focal length figure}

pdf(file = "Figures/LE2_back_focal.pdf", width = 8.8,  height = 8)
LE2_focal
dev.off()

```

```{r supplementary back focal length figure}
...
fig.b <- RE1_focal +
  theme(axis.title.y = element_blank()) + 
  theme(legend.position = "none",
        plot.margin = unit(c(0, 0, 0.5, 0), "cm")) #The order of edges for plot.margin is unit(c(top, right, bottom, left), units)
...
```{r left right asymmetry supplementary figure}

fig.a <- E1_LR_control_oph_plot  #The order of edges for plot.margin is unit(c(top, right, bottom, left), units)
...
# Creating common axis labels
y.grob <- textGrob("Reciprocal object distance", 
                   gp = gpar(fontface = "bold",
                             fontsize = 25), rot = 90, vjust = 0.5)

x.grob <- textGrob("Reciprocal object distance",
                   gp = gpar(fontface = "bold",
                             fontsize = 25))
...
legend_large <- get_legend(LE1_edge_plot + 
              ...
              legend.text = element_text(size=16))) #change legend text font size) # legend is laid out horizontally
