...
authors: "Mitra AT., Rathore S., Jester A., Hyland-Brown R., Hassert J., Benoit JB., Stowasser A, Tomoyasu Y., Buschbeck EK."
date: "7 April 2024"
output:
  html_document:
...
# Hanging drops
edge_data <- read.csv("lens3_edge_sharpness_long.csv", header = TRUE)

hang_data <- read.csv("lens3_hanging_drops.csv", header = TRUE)
...
# Figures
For the code below to work, create a folder called "Figures" in the same place as the RProject and RMarkdown file
...
pdf(file = "Figures/qPCR.pdf", width = 9,  height = 6)
qPCR_plot
dev.off()
...
# Melting the data by distance to make it long format:
# edge_data <- melt(edge_data, id.vars = "distance", value.name = "contrast")

# Pulling information from rows and trying to organise it into a new column:
# edge_data_long$left <- str_detect(edge_data_long$variable, "L") 
```
...
```{r edge sharpness curves figure}
...
edge_supplement_plot <- plot_grid(fig.a, fig.b, fig.c, fig.d,
...
plot_grid(edge_supplement_plot)

edge_supplement_xlab <- plot_grid(edge_supplement_plot, x.grob,
                                 ncol = 1, rel_heights = c(1, .1))
plot_grid(edge_supplement_xlab)

# Print graph
cairo_pdf(file = "Figures/edge_curve_supplement.pdf", width = 12,  height = 8) # cairo_pdf is able to handle special characters
grid.arrange(arrangeGrob(edge_supplement_xlab, 
                         left = y.grob,
                         bottom = legend))
dev.off()
...
```{r maximum edge sharpness boxplots figure}
...
# Creating common axis labels
y.grob <- textGrob("Maximum relative edge sharpness", 
                   gp = gpar(fontface = "bold",
                             fontsize = 25), rot = 90,
                             vjust = 0.6,
                             hjust = 0.5)
...
# Use cowplot package to nicely plot the graphs together 
max_sharpness_plot <- plot_grid(fig.a, fig.b, fig.c, fig.d,
                   #list of plots to arrange in grid
                   rel_widths = c(6, 6),
                   align = "v",
                   labels = c("A","B","C","D"), # panel labels for figure
...
plot_grid(max_sharpness_plot)

# Print graph
cairo_pdf(file = "Figures/max_sharpness_supplement.pdf", width = 12,  height = 8) # cairo_pdf is able to handle special characters
grid.arrange(arrangeGrob(max_sharpness_plot, left = y.grob))
dev.off()
...
```{r back focal length boxplot figure}
...
# Use cowplot package to nicely plot the graphs together 
focal_supplement_plot <- plot_grid(fig.a, fig.b, fig.c, fig.d,
                   #list of plots to arrange in grid
                   rel_widths = c(6, 6),
                   labels = c("A","B","C","D"), # panel labels for figure
                   label_size = 23,
...
plot_grid(focal_supplement_plot)

# Print graph
cairo_pdf(file = "Figures/focal_supplement_plot.pdf", width = 13,  height = 8) # cairo_pdf is able to handle special characters
grid.arrange(arrangeGrob(focal_supplement_plot, left = y.grob))
dev.off()
```

```{r figure 5: sharpness curve & maximum sharpness & focal length}
...
# writing to pdf
pdf(file = "Figures/LE2_hanging_drop.pdf", width = 20,  height = 10)
fig.fgh
dev.off()
...
# writing to pdf
pdf(file = "Figures/hanging_drops_supplement.pdf", width = 19,  height = 14)
final.supplement
dev.off()
...
qqnorm(LR_E1_control_mod$residuals, pch = 1, frame = FALSE)
qqline(LR_E1_control_mod$residuals, col = "steelblue", lwd = 2)
```
...
# Print graph
cairo_pdf(file = "Figures/LvR_asymmetry_supplement.pdf", width = 23,  height = 13) # cairo_pdf is able to handle special characters
...
behaviour_plots <- grid.arrange(arrangeGrob(latency_plot_axis, hunting_plot_axis, nrow = 2))
...
behaviour_final <- ggdraw() +
  ...
  draw_image(image = larva_h_icon, x = 0.41, y =  0.35, scale = 0.13) +
  draw_image(image = larva_h_icon, x = 0.41, y =  -0.11, scale = 0.13) +
  # vertical larva silhouettes
  draw_image(image = larva_v_icon, x = 0.41, y =  0.11, scale = 0.13) +
  draw_image(image = larva_v_icon, x = 0.41, y =  -0.35, scale = 0.13) +
  # bright & dim light vectors
  draw_image(image = bright_icon, x = -0.2405, y =  0.45, scale = 0.083) +
  draw_image(image = dim_icon, x = 0.165, y =  0.441, scale = 0.065)
...
# Defining nm_to_RGB. 
# This is a function converts a wavelength of light into a hex-string to represent a colour
...
### with colour gradient
## bright
# horizontal 
horizontal_bright_spec_colour <- spec_data %>%
...
# vertical
vertical_bright_spec_colour <- spec_data %>%
...
## dark
# horizontal 
horizontal_dark_spec_colour <- spec_data %>%
  filter(arena == "horizontal" & light_environment == "dark") %>%
  ggplot(aes(x = wavelength, y = absolute_irradiance)) +
  geom_spline() +
  scale_x_continuous(limits = c(300, 750), 
...
## dark
# vertical
vertical_dark_spec_colour <- spec_data %>%
  filter(arena == "vertical" & light_environment == "dark") %>%
  ggplot(aes(x = wavelength, y = absolute_irradiance)) +
  geom_segment(aes(xend = wavelength, yend = 0, 
                   colour = nm_to_RGB(wavelength))) +
...
# find coordinates of spline ## That's the way to do this
# other smoothers - LOESS - outside of ggplot
# fill polygon is underneath line 
# as the to plot layer overplot a white polygon that goes over the top 
# 
...
# Making panels
fig.a <- horizontal_bright_spec +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank())

fig.b <- vertical_bright_spec +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank())

fig.c <- horizontal_dark_spec +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank())

fig.d <- vertical_dark_spec +
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank())
...
spec_figure_final<- ggdraw() +
  draw_plot(spec_figure_margins) +
    # bright & dim light vectors
    draw_image(image = bright_icon, x = 0.47, y =  0.26, scale = 0.083) +
    draw_image(image = dim_icon, x = 0.47, y =  -0.2, scale = 0.065) +
    # horizontal larva silhouettes
...
# Print graph
cairo_pdf(file = "Figures/behaviour_spectra_supplement.pdf", width = 18,  height = 12) # cairo_pdf is able to handle special characters\
spec_figure_final
dev.off()
...
```{r normal light behaviour latency stats pooled probes}
...
```{r dark behaviour hunting success pooled probes}
