...
# Densitometry
densitom_data <- read.csv("densitometry.csv", header = TRUE)

densitom_extracted_data <- read.csv("densitometry_extracted.csv", header = TRUE)
...
Quantifying opacities with digital evaluation of lens back surface images. This method is adapted from Seeberger et al., 2004. Data is generated by selecting a single diametric line through the centre of the lens on ImageJ and using the plot profile function to generate grey values / pixel brightness values along all pixels on the selected line.

```{r densitometry curves & boxplots}

# Creating new column with the eye and probe identity
densitom_data <- densitom_data %>%
  mutate(eye_probe_individual = paste(eye, probe, individual, sep="_"))

## Individual grey value curves

# Control lens
density_plot_control <- densitom_data %>%
  filter(eye_probe_individual == "LE2_control_C18") %>%
  ggplot(aes(y= gray_value, x = distance_pixels)) +
  geom_spline(spar = 0.3) +
  scale_y_continuous("Grey value", 
...
density_plot_control

# Probe 1 lens
density_plot_P1 <- densitom_data %>%
  filter(eye_probe_individual == "RE2_lens3_191_191_21") %>%
  ggplot(aes(y= gray_value, x = distance_pixels)) +
  geom_spline(spar = 0.3) +
  scale_y_continuous("Grey value", 
                     breaks = c(20, 40, 60, 80), 
...
# Probe 2 lens
density_plot_P2 <- densitom_data %>%
  filter(eye_probe_individual == "RE2_lens3_249_249_18") %>%
  ggplot(aes(y= gray_value, x = distance_pixels)) +
  geom_spline(spar = 0.3) +
  scale_y_continuous("Grey value", 
                     breaks = c(20, 40, 60, 80), 
...
## boxplots of centre and periphery grey values

# Creating new column with average grey values for periphery
densitom_extracted_data <- densitom_extracted_data %>%
  rowwise() %>%
  mutate(lens_periphery_grey_value =
           mean(c_across(c('lens_periphery_1_grey_value', 'lens_periphery_2_grey_value'))))

# Lens centre
centre_densito <- densitom_extracted_data %>%
  ggplot(aes(y= lens_centre_grey_value, x = probe, fill = probe)) +
  geom_boxplot(alpha = 0.8, outlier.shape = NA) +
  scale_fill_manual(values=c("#009E73", "#E69F00", "#D55E00")) +
  scale_x_discrete(labels = c(expression("Control", 
                                         paste(italic("lens3"), "RNAi P1"), 
                                         paste(italic("lens3"), "RNAi P2")))) +
  scale_y_continuous(limits = c(10, 90), breaks = c(20, 40, 60, 80)) +
  labs(title=NULL,x = NULL, y = "Grey value") +
  geom_jitter(shape = 19, size = 3.5, alpha = 0.7, position = position_jitter(0.1)) +
  geom_signif(comparisons=list(c("control", "lens3_249")), annotations="*", # adds a significance star
              map_signif_level = TRUE, textsize = 10, 
              vjust = 0.5,
...
# Lens surround
surround_densito <- densitom_extracted_data %>%
  ggplot(aes(y= lens_periphery_grey_value, x = probe, fill = probe)) +
  geom_boxplot(alpha = 0.8, outlier.shape = NA) +
  scale_fill_manual(values=c("#009E73", "#E69F00", "#D55E00")) +
  scale_x_discrete(labels = c(expression("Control", 
                                         paste(italic("lens3"), "RNAi P1"), 
...
```{r figure S1: densitometry}

###### individual grey value curves ######

fig.a <- density_plot_control +
  theme(axis.title.y = element_blank(),
        axis.title.x = element_blank()) + 
  theme(legend.position = "none",
        plot.margin = unit(c(0.2, 0, 0.5, 0.7), "cm")) #The order of edges for plot.margin is unit(c(top, right, bottom, left), units)

fig.a

fig.b <- density_plot_P1 +
  theme(axis.title.y = element_blank(),
        axis.title.x = element_blank()) + 
  theme(legend.position = "none",
        plot.margin = unit(c(0.2, 0, 0.5, 0.7), "cm"))
  
fig.b

fig.c <- density_plot_P2 +
  theme(axis.title.y = element_blank(),
        axis.title.x = element_blank()) + 
  theme(legend.position = "none",
        plot.margin = unit(c(0.2, 0, 0.5, 0.7), "cm"))
...
densitometry_plot <- plot_grid(fig.a, fig.b, fig.c,
                   ...
                   vjust = 0.9,
                   nrow = 1) +
  theme(plot.margin = unit(c(10, 20, 5, 5), "points")) # The order of edges for plot.margin is unit(c(top, right, bottom, left), units)

densitometry_final <- grid.arrange(arrangeGrob(densitometry_plot, left = y.grob))

plot_grid(densitometry_final)

# Print
cairo_pdf(file = "Figures/densitometry_supplement.pdf", width = 20,  height = 5) # cairo_pdf is able to handle special characters
grid.arrange(arrangeGrob(densitometry_final))
dev.off()

###### boxplots of extracted values ######

fig.a <- centre_densito +
  theme(axis.title.y = element_blank(),
        axis.title.x = element_blank()) + 
  theme(legend.position = "none",
...
fig.a <- ggdraw(fig.a) + 
  draw_label("Centre",
             size = 20,
             y = 0.045, # y coordinates
             x = 0.55,
             fontface = 'bold') 
fig.a


fig.b <- surround_densito +
  theme(axis.title.y = element_blank(),
        axis.title.x = element_blank()) + 
  theme(legend.position = "none",
...
grey_value_plot <- plot_grid(fig.a, fig.b,
                   #list of plots to arrange in grid
                   ...
                   align = "v",
                   nrow = 1) +
  theme(plot.margin = unit(c(15, 20, 5, 20), "points")) # The order of edges for plot.margin is unit(c(top, right, bottom, left), units)

plot_grid(grey_value_plot)

grey_value_plot_final <- grid.arrange(arrangeGrob(grey_value_plot, 
                         left = y.grob))

plot_grid(grey_value_plot_final)

# Print
cairo_pdf(file = "Figures/grey_value_supplement.pdf", width = 20,  height = 7) # cairo_pdf is able to handle special characters
grid.arrange(arrangeGrob(grey_value_plot_final))
dev.off()
```

# Figure S2
```{r figure S2: hanging drops}
...
# writing to pdf
pdf(file = "Figures/figure_S2.pdf", width = 19,  height = 14)
final.supplement
dev.off()
...
# Building the linear model
centre_densitom_model <- lm(lens_centre_grey_value ~ probe, data = densitom_extracted_data)

# Creating a QQ plot of residuals
ggqqplot(residuals(centre_densitom_model))

# Test for normality
shapiro_test(residuals(centre_densitom_model))
# p > 0.05 so assuming normality

# Checking normality assumption by groups
densitom_extracted_data %>%
  group_by(probe) %>%
  shapiro_test(lens_centre_grey_value)
# again p > 0.05 so assuming normality

# Creating QQ plots for each group level
ggqqplot(densitom_extracted_data, "lens_centre_grey_value", facet.by = "probe")
# Looks normal; will proceed with Gaussian stats

# Using an ANOVA test
centre_densitom_aov <- compare_means(lens_centre_grey_value ~ probe, data = densitom_extracted_data, method = 'anova')
centre_densitom_aov

centre_densitom_aov <- densitom_extracted_data %>%
  anova_test(lens_centre_grey_value ~ probe)

centre_densitom_aov

summary(centre_densitom_aov)
# Significant difference in grey values between control and knockdown lens centres
...
# Building the linear model
surround_densitom_model <- lm(lens_periphery_grey_value ~ probe, data = densitom_extracted_data)

# Creating a QQ plot of residuals
ggqqplot(residuals(surround_densitom_model))

# Test for normality
shapiro_test(residuals(surround_densitom_model))
# p > 0.05 so assuming normality

# Checking normality assumption by groups
densitom_extracted_data %>%
  group_by(probe) %>%
  shapiro_test(lens_periphery_grey_value)
# again p > 0.05 so assuming normality

# Creating QQ plots for each group level
ggqqplot(densitom_extracted_data, "lens_periphery_grey_value", facet.by = "probe")
# Looks normal; will proceed with Gaussian stats

# Using an ANOVA test
surround_densitom_aov <- compare_means(lens_periphery_grey_value ~ probe, data = densitom_extracted_data, method = 'anova')
surround_densitom_aov

surround_densitom_aov <- densitom_extracted_data %>%
  anova_test(lens_periphery_grey_value ~ probe)

surround_densitom_aov

summary(surround_densitom_aov)
# Significant difference in grey values between control and knockdown lens centres
