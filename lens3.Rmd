...
# Test for normality
shapiro.test(oph_data$bestimagereciprocal)

```

Can't really use the following code because treating the left and right eyes as individual data points would be pseudo-replication...

oph_test <- oph_data %>%
  filter(probe != "lens3_191") %>%
  filter(eye == "LE1" | eye == "RE1") 

t.test(oph_test$bestimagereciprocal ~ oph_test$probe)  
